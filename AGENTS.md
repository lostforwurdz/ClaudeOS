...
- Optional MCPs (auto-injected when env vars are set; no-op otherwise):
  - `auggie` (Augment Context Engine, repo-scale semantic RAG): `npm install -g @augmentcode/auggie@latest`, `auggie login`, then export `CLAUDEOS_AUGMENT_SESSION_AUTH="$(auggie token print)"` before `npm run desktop:dev`. Free tier offers 1,000 queries during launch.
- Quality hooks (lint/test/etc.): add `PostToolUse` and `Stop` hook entries to your `~/.claude/settings.json` or per-project `.claude/settings.json`. Claude Code merges hook arrays across settings scopes, so they fire during ClaudeOS runs alongside the ClaudeOS-injected permission hook (xh4.2). See https://code.claude.com/docs/en/hooks. ClaudeOS does not yet read or render hook stderr — that's a follow-up.
